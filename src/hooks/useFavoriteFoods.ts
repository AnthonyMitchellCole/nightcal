...
// Query key factory
const favoriteFoodsKeys = {
  all: ['favoriteFoods'] as const,
  byUser: (userId: string) => [...favoriteFoodsKeys.all, userId] as const,
};

// Fetch favorite foods based on user's food logs
const fetchFavoriteFoods = async (userId: string): Promise<FavoriteFood[]> => {
...
export const useFavoriteFoods = () => {
  ...
  const {
    ...
    queryKey: favoriteFoodsKeys.byUser(user?.id || ''),
    queryFn: () => fetchFavoriteFoods(user!.id),
    enabled: !!user,
    staleTime: 10 * 60 * 1000, // Favorite foods cached for 10 minutes
    gcTime: 30 * 60 * 1000, // Keep in memory for 30 minutes
  });

  const error = queryError instanceof Error ? queryError.message : null;

  return { favoriteFoods, loading, error };
};